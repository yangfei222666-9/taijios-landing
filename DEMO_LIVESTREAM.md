...
> "这个叫**诸葛亮推演军师**，是 TaijiOS 里的一个技能。核心卖点一句话——**把可规则化的推演先放在本地结构化完成，LLM 只在需要生成解释时调用**。不是玄学，是把状态判断、证据整理和解释生成分层。"
...
> "这段古文是 DeepSeek 实时生成的，基于我刚才的 6 爻和卦象。每次都不一样，孙子兵法味很浓。这里的关键不是承诺固定省多少 token，而是把结构化推演和文字生成拆开，后续可以用实际调用日志做成本对比。"
